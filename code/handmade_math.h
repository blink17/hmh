...
inline v2
Perp(v2 A)  // Perpendicular // 수직의, 직교한
{
    v2 Result = {-A.y, A.x};    // 회전공식은 x' = x*cos - y*sin; y' = y*cos + x*sin 이므로
    return(Result);             // 수직은 90도 회전이므로 cos90 = 0, sin90 = 1; 그러므로 x' = -y; y' = x;
}
...
inline v2
GetBarycentric(rectangle2 A, v2 P)  // Barycentric 무게중심이라는 뜻이다.
{
    v2 Result;
