...
GetHashFromStorageIndex(sim_region *SimRegion, uint32 StorageIndex)
{
    ...
    for(uint32 Offset = 0;
        ...
        ++Offset)
    {
        uint32 HashMask = (ArrayCount(SimRegion->Hash) - 1); // 이렇게 범위를 벗어나지 않게 구하기 위해서 2의승으로 Hash의 배열개수를 맞춤. 근데 이거 루프들어가기전에 미리 계산해 놓는게 빠르지 않나
...
GetSimSpaceP(sim_region *SimRegion, low_entity *Stored)
{
    ...
    {
        Result = Subtract(SimRegion->World, &Stored->P, &SimRegion->Origin); // Region의 오리진기준 로컬좌표를 구하는것과 같다.
    }    
...
EndSim(sim_region *Region, game_state *GameState)
{
        ...
        ++EntityIndex, ++Entity)
    {
        ...
        Assert(IsSet(&Stored->Sim, EntityFlag_Simming));    // BeginSim에서 Simming으로 설정했으니깐 설정되어있어야된다.
        Stored->Sim = *Entity;  // 시뮬레이션한 엔티티 데이터로 교체
        Assert(!IsSet(&Stored->Sim, EntityFlag_Simming));   // 복사된 Sim은 Simming 플래그가 없으니깐 검사한다.

        StoreEntityReference(&Stored->Sim.Sword);   // Ptr -> Index로 다시 변환
         ...
         real32 *tMin, real32 MinY, real32 MaxY)
{
    ...
    {
        real32 tResult = (WallX - RelX) / PlayerDeltaX; // ps=p0+ds 에서 가중치 s를 구하기 위해, s = (ps-p0)/d 로 s(tResult)구함
        real32 Y = RelY + tResult*PlayerDeltaY;         // 위에서 x를 통해서 WallX에 도달하기 위한 가중치를 구한걸로 도착지점 Y를 구함
        if((tResult >= 0.0f) && (*tMin > tResult))      // 0 <= tResult < 1.0 사이면 이동을 완료하기 전에 벽에 도달했다는 것이니깐 충돌이다.
        {                                               // 1.0 < tResult 이라면 벽에 도달하기위해서 현재이동값인 d보다 더 이동해야된다는거니깐 벽에 도달하지 못한것이다.
            if((Y >= MinY) && (Y <= MaxY))              // tResult < 0 이라면 이동방향 d의 반대편으로 이동해야된다는거니깐 이미 벽을 등진상태.
            {
           ...
           move_spec *MoveSpec, v3 ddP)
{
    ...
    if(MoveSpec->UnitMaxAccelVector)
    {
        ...
        if(ddPLength > 1.0f)
        {
            ddP *= (1.0f / SquareRoot(ddPLength));  // 정규화. Unit(노멀)벡터
        }
    }

    ddP *= MoveSpec->Speed;     // Speed(속력 - 방향이 없는 스칼라 타입이므로 -)라기 보다는 AccelerationLenth에 가깝다.
    ...
    v3 PlayerDelta = (0.5f*ddP*Square(dt) + Entity->dP*dt); // dt = t-t0, x = 0.5*a*dt^2 + vt + x0                     
    Entity->dP = ddP*dt + Entity->dP;     // v = a*dt + v0  <---  a = (v-v0)/dt
