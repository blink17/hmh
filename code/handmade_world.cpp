...
inline bool32
IsCanonical(real32 ChunkDim, real32 TileRel)   // Chunk단위 검사이다. 변수명이나 함수명이 설명이 충분치 않음.
{
...
inline bool32
IsCanonical(world *World, v3 Offset)        // Chunk단위 검사이다. 변수명이나 함수명이 설명이 충분치 않음.
{
...
RecanonicalizeCoord(real32 ChunkDim, int32 *Tile, real32 *TileRel)
{
    ...
    int32 Offset = RoundReal32ToInt32(*TileRel / ChunkDim); // 개놈이 Tile이 아니라 Chunk단위 잖어 죽을래 네이밍
    *Tile += Offset;
